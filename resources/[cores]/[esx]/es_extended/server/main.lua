...
local newPlayer = "INSERT INTO `users` SET `accounts` = ?, `identifier` = ?, `group` = ?, `citizenid` = ?"
local loadPlayer = "SELECT `accounts`, `job`, `job_grade`, `group`, `position`, `inventory`, `skin`, `loadout`, `metadata`, `citizenid`"
...
function createESXPlayer(identifier, playerId, data)
    ...
    local bsnNumber = exports['ml-core']:createBSN()
    local parameters = { json.encode(accounts), identifier, defaultGroup, bsnNumber }
...
function loadESXPlayer(identifier, playerId, isNew)
    ...
    userData.citizenid = result.citizenid
    ...
    -- xPlayer Creation
    local xPlayer = CreateExtendedPlayer(playerId, identifier, userData.group, userData.accounts, userData.inventory, userData.weight, userData.job, userData.loadout, GetPlayerName(playerId), userData.coords, userData.metadata, userData.citizenid)
...
AddEventHandler("playerDropped", function(reason)
    ...
    if xPlayer then
        TriggerEvent("esx:playerDropped", playerId, reason, xPlayer)
